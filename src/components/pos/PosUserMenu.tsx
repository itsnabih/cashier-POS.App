...
export function PosUserMenu({ fullName, role }: PosUserMenuProps) {
  ...
  return (
    <div className="flex items-center gap-3 bg-baby-800/50 px-3 py-1 rounded-lg border border-baby-500/30" suppressHydrationWarning>
      <div className="text-right hidden sm:block">
        <div className="text-xs font-semibold text-white">{fullName}</div>
        <div className="text-[10px] text-baby-200 capitalize leading-none">{role}</div>
      </div>
      <div className="h-6 w-px bg-baby-500/50 hidden sm:block"></div>
      <button
